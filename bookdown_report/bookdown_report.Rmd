...
```
 Results from Model \@ref(eq:glmm) and Model \@ref(eq:bayes) were congruent, as shown in Table \@ref(tab:est_tab). The 95% highest posterior density (HPD) credible intervals from Model \@ref(eq:bayes) offer the advantage of concluding 95% confidence that estimates for each parameter lie within the HPD interval.  
...
rrplot1 <- ggplot(rrs1, aes(x = ratio, y = Contrast, xmin = Lower, xmax = Upper)) +
  ...
  labs(x = "Relative Risk",
       title = "Within Sex, Between Treatment")
...
rrplot2 <- ggplot(rrs2, aes(x = ratio, y = Contrast, xmin = Lower, xmax = Upper)) +
  ...
  scale_color_startrek() +
  labs(x = "Relative Risk", title = "Within Treatment, Between Sex")
par(mar = c(4, 4, .2, .2));
rrplot1
rrplot2
```  
