...
knitr::opts_chunk$set(fig.width=4, fig.height=4, fig.pos = 'H',
...
Little is known about the effects of high atomic number and energy (HZE) radiation, a main component of space radiation to which exposure is unavoidable beyond Earth's magnetic field. In contrast, extensive research shows exposure to high doses of gamma radiation leads to acute radiation sickness. One known effect of radiation exposure is cataract development. The serious health implications from HZE radiation for astronauts who leave Earth's magnetic field warrant further study. Adverse effects from radiation may also be attributable to other factors, including genetics; several different cancers have been observed to cluster in mice families (Chernyavskiy, et. al., 2017). Accounting for potential family clustering allows for thorough examination of the primary research questions. Is there a genetic susceptibility to cataracts caused by radiation? Accounting for potential genetic susceptibility, is there a difference in cataract presentation between HZE radiation and gamma radiation? The full dataset includes 1820 unique mice from 48 unique families, with equal random assignments by family to each of three treatment groups. Mice are bred over several generations to create a genetically heterogeneous sample, the better to represent the diverse biology of the human population. The treatments are HZE irradiation, gamma irradiation, and non-irradiated control. Mice in both irradiated groups are exposed to radiation at 7-12 weeks of age. All mice are monitored until 800 days of age -effectively a survival study. This analysis uses a simplified version of the data set - a snapshot of the 1169 mice that are alive at 552 days. This cutoff is chosen because it is the median survival time for the group with the shortest median survival. There are 47 unique families with n = 396 in the HZE group, n = 277 in the gamma radiation group, and n = 496 in the unirradiated control group. Family size ranges from n = 11 to n = 48, with median family size of 24.  
...
```{r lineeda, fig.cap="Average family cataract score by sex, treatment group"}
...
Assumptions for fitting this model were: the random effects must come from a normal distribution, the chosen link function is appropriate, and estimation of variance is appropriate (no over-dispersion). 
These assumptions were checked and met as follows: quantiles of the random effect were compared with the quantiles of a normal distribution and looked approximately normal **(this plot is in the appendix. Not sure if it should go in the acutal report?)**, the chosen link function was a logit which is appropriate because the outcome is binary, and finally, the ratio of the chi square statistic to the residual degrees of freedom is less than 1 (.89) which indicates that there is no over-dispersion.
  
```{r final_mod}
mod <- glmer(Cataracts ~ Treatment*Sex + (1|Family), data = cats, family = binomial)
```

A complementary Bayesian model with non-informative priors was fit to examine the distribution of the estimates from the final model, as well as test its robustness to alternative approaches [@BDA]. The Bayes model is parameterized as:  
\begin{equation}
\begin{aligned}
Y_{ij} \sim &Bernoulli(p)\\
...
xtable(est_tab_show, caption = "Final model parameter estimates") %>%
  xtable2kable(booktabs = T, include.rownames = FALSE, table.placement = NULL) %>%
  kable_styling(latex_options = "HOLD_position") 
```
 
The 95% highest posterior density (HPD) credible intervals from Model \@ref(eq:bayes)

\section{Limitations and Alternatives}
\label{sec:limits}
 
Logistic regression assumes linearity of $log(\frac{p}{1-p})$ [@BMLR2021]. 
Limitations include lack of robust diagnostic assessment of GLMMs [@CDA]. 
...
Comparisons of different groups across Sex and Treatment were assessed on Model \@ref(eq:glmm) using the `emmeans` package [@R-emmeans]. Figure \@ref(fig:contr) shows the probability of developing cataracts for each combination of sex by treatment group. Differences between both sex and treatment group are clearly visible, notably the differences between treatment group across gender.
  
```{r contr, fig.cap="Predicted probability of cataracts by sex, treatment group", fig.align='center'}
...
```{r RR, fig.show='hold', fig.cap='Relative risk of cataract development between all group combinations', , out.width='50%', fig.fullwidth=TRUE}
...
Figure \@ref(fig:RR) shows the relative risk of cataract development for all groups in Model \@ref(eq:glmm). From the plot on the left we see that mice in the HZE group had a higher risk of developing severe cataracts than both the gamma and control groups. Males in the HZE group had 1.36 times greater risk than males in the gamma group. The biggest difference is seen in the HZE group compared to the unirradiated control group for males. Males in HZE had 3.37 times greater risk than males in the control group. On the right, it is made clear that males have a much higher risk of developing cataracts than females. In the HZE group, males have a 4.74 times higher risk of developing severe cataracts than females and in the gamma group it is a 4.54 times higher risk.

Fix this plot! 

Visualizing random effect by family:  
```{r re_plot, fig.cap="Estimated random probability of developing cataracts by family", fig.align = 'center}
p_sigs <- est_tab[7,]
